...
async function fetchEntries() {
  ...
  try {
    ...
    balanceText.classList.remove('positive', 'negative'); // Hapus kelas sebelumnya
    if (balance >= 0) {
...
function renderEntry(item, type, listElement) {
  ...
  // Bagian 1: Baris Ringkasan (Amount, Date, Category)
  const topRow = document.createElement("div");
  topRow.className = "entry-summary-top-row";

  const amountSpan = document.createElement("span");
  amountSpan.className = "amount";
  amountSpan.textContent = formatRupiah(item.amount);

  const dateAndCategoryDiv = document.createElement("div");
  dateAndCategoryDiv.innerHTML = `
      <span class="date">${item.date || "-"}</span>
      <span class="category">${item.category || "-"}</span>
  `;

  topRow.appendChild(amountSpan);
  topRow.appendChild(dateAndCategoryDiv);
  li.appendChild(topRow);

  // Bagian 2: Detail (Description, Category, Date) - Awalnya Tersembunyi
  const detailContent = document.createElement("div");
  detailContent.className = "entry-details";
  detailContent.innerHTML = `
      <strong>Description:</strong> ${item.description || "-"}<br/>
      <strong>Category:</strong> ${item.category || "-"}<br/>
      <strong>Date:</strong> ${item.date || "-"}
  `;
  li.appendChild(detailContent);

  // Bagian 3: Tombol Edit dan Delete
  const btnGroup = document.createElement("div");
  btnGroup.className = "entry-action-buttons";

  const deleteBtn = document.createElement("button");
  deleteBtn.className = "btn btn-danger me-2";
  deleteBtn.textContent = "Delete";
  deleteBtn.onclick = async (e) => {
    e.stopPropagation(); // Mencegah event klik menyebar ke li
    try {
  ...
  editBtn.onclick = async (e) => {
    ...
    try {
      ...
      console.error("Failed to delete entry for edit:", err);
      alert("Failed to prepare for edit. Original entry not removed.");
    }
  };
  ...
  // Logika On-Click untuk Toggling Detail (tidak menutup item lain secara otomatis)
